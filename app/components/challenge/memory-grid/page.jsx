...
export default function MemoryGridChallenge() {
  ...
  const handleAllocation = (blockId) => {
    ...
    // Basic Validation: Check if it fits and is empty
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex items-center justify-center font-mono">
      <div className="text-emerald-500 animate-pulse text-[10px] tracking-[0.5em] uppercase">Mapping_Virtual_Address_Space...</div>
    </div>
  );

  if (gameState === "summary") return (
    <div className="min-h-screen bg-[#020202] flex items-center justify-center font-mono p-6">
      <div className="max-w-xl w-full border border-white/5 p-16 bg-[#080808] rounded-[4rem] text-center shadow-2xl relative overflow-hidden">
        <div className="absolute -top-24 -left-24 w-48 h-48 bg-emerald-500/10 blur-[100px]"></div>
        <h1 className="text-6xl font-black text-white italic mb-4 tracking-tighter uppercase leading-none">Allocation_End</h1>
        <p className="text-emerald-500 text-[10px] font-black uppercase tracking-[0.4em] mb-12">Total Efficiency XP: {score}</p>
        <div className="grid gap-4">
          ...
          <Link href="/challenges" className="w-full py-6 bg-transparent border border-white/10 text-white font-black rounded-2xl uppercase tracking-widest flex items-center justify-center text-sm">Return_to_Home</Link>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] text-white font-mono p-6 md:p-12">
      <div className="max-w-7xl mx-auto">
        <header className="flex justify-between items-start mb-16 border-b border-white/10 pb-10">
          <div>
            <h1 className="text-4xl font-black italic tracking-tighter uppercase mb-2 underline decoration-emerald-500 decoration-4">Memory_Grid</h1>
          ...
          <div className="bg-white/5 px-8 py-4 rounded-2xl border border-white/5 text-right">
            <p className="text-[10px] text-gray-500 font-black mb-1 uppercase">Points</p>
            <p className="text-4xl font-black text-emerald-500 tabular-nums">{score}</p>
          </div>
        </header>
            ...
            <div className="space-y-4">
              {pendingProcesses.map((p, i) => (
                <div key={p.id} className={`p-6 border-2 rounded-2xl transition-all duration-500 ${i === 0 ? 'border-emerald-500 bg-emerald-500/10 scale-105 shadow-xl shadow-emerald-500/5' : 'border-white/5 opacity-30'}`}>
                  <p className="text-[9px] font-black text-emerald-500/50 mb-1">TASK_{p.id}</p>
                  <p className="text-2xl font-black italic text-white">{p.name}</p>
              ...
              {memoryBlocks.map((block) => (
                ...
                >
                  <div className="flex justify-between items-center relative z-10">
                    <div>
                        <p className="text-[9px] font-black text-gray-600 mb-1 tracking-tighter">BLOCK_ADDR_{block.id}</p>
                    ...
                    {block.occupant ? (
                        <div className="text-right">
                            ...
                            <p className="text-xl font-black italic text-emerald-500">{block.occupant.name}</p>
                            <p className="text-[10px] text-gray-600 font-bold italic">Gap: {block.size - block.occupant.size} KB</p>
                        </div>
                    ) : (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="text-[10px] font-black bg-white text-black px-6 py-3 rounded-full uppercase tracking-tighter">Assign_Task</span>
                        </div>
                    )}
                  </div>
                  {block.occupant && (
                      <div className="absolute inset-0 bg-emerald-500/5 transition-all" />
                  )}
                </button>
              ))}
            </div>
      ...
      {gameState === "success" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-3xl p-6">
          <div className="max-w-md w-full bg-[#080808] border border-white/10 p-12 rounded-[4rem] text-center shadow-2xl relative">
            <h2 className="text-5xl font-black text-white italic mb-2 uppercase tracking-tighter">{hasSkipped ? "Overridden" : "Synchronized"}</h2>
