...
export default function ChallengePage() {
  return (
    <div className="min-h-screen bg-[#050505] text-white font-mono p-8 md:p-20 overflow-hidden relative">
      {/* Background Decor */}
      <div className="fixed inset-0 pointer-events-none opacity-[0.02] z-0 bg-size-[50px_50px] bg-[linear-gradient(to_right,#808080_1px,transparent_1px),linear-gradient(to_bottom,#808080_1px,transparent_1px)]"></div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {CHALLENGES.map((challenge) => (
            <Link 
              key={challenge.id} 
              href={challenge.path}
              className="group relative block bg-[#0a0a0a] border border-white/5 p-8 rounded-[2.5rem] hover:border-emerald-500/50 hover:bg-emerald-500/2 transition-all duration-500 overflow-hidden"
            >
              {/* Animated Glow */}
